...
def run_episode(env_name, idx, port, args, tokenizer=None, model_name=None, save_traces=False, run_id=None):  # noqa: C901
    ...
    use_session = args.mode not in ("baseline", "summary", "markovian_pure") and (
        args.use_sessions or args.mode in ("compaction", "compaction_full", "markovian")
    )
    session_id = f"{run_id}_{env_name}_{idx}" if use_session else None
    ...
    for turn in range(args.max_turns):
        ...
        if args.mode == "summary" and cumulative_summary:
            messages = build_messages_with_summary(
                cumulative_summary, history, obs_text,
                no_thinking=args.no_thinking,
            )
        else:
            messages = build_messages(history, obs_text, no_thinking=args.no_thinking)

        # Context-length cap: stop episode if prompt + response would exceed budget
        if args.max_episode_tokens and tokenizer and args.mode in ("baseline", "summary", "markovian_pure"):
            prompt_ids = tokenizer.apply_chat_template(messages, add_generation_prompt=True)
            if hasattr(prompt_ids, "keys"):
                prompt_ids = prompt_ids["input_ids"]
            if len(prompt_ids) + args.max_response_tokens > args.max_episode_tokens:
                break
        ...
        if args.mode in ("baseline", "summary", "markovian_pure"):
            ...
            if session_token_ids is None:
                ...
                resp = client.post("/compact_session/create", json={
                    ...
                    "top_p": args.top_p,
                    "compaction_mode": {"markovian": "markovian", "compaction_full": "attention_matching_full"}.get(args.mode, "attention_matching"),
            ...
            elif args.mode == "compaction_full":
                body["compaction_mode"] = "attention_matching_full"
...
def main():
    ...
    parser.add_argument("--mode", choices=["baseline", "compaction", "compaction_full", "markovian", "summary", "markovian_pure"], required=True)
    ...
    parser.add_argument("--max-episode-tokens", type=int, default=None,
                        help="Stop episode when prompt + max_response_tokens would exceed this limit")
    parser.add_argument("--summary-max-tokens", type=int, default=200,
                        help="Max tokens for summary generation (summary mode only)")
    args = parser.parse_args()

    if args.mode in ("summary", "markovian_pure", "compaction_full") and args.n_max_turns < 0:
        args.n_max_turns = 4
    if args.mode in ("summary", "markovian_pure", "compaction_full") and args.n_preserved_turns == 0:
